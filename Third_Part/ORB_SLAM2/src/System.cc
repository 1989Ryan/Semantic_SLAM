...
cv::Mat System::TrackMonocular(const cv::Mat &im, const cv::Mat &ca, const double &timestamp)
{
    if(mSensor!=MONOCULAR)
    {
    ...
    cv::Mat Tcw = mpTracker->GrabImageMonocular(im,ca, timestamp);
